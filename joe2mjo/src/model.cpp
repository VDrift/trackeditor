...
bool JOEMODEL::Load(string strFileName, bool autoloadtexture)
{
    ...
    FILE * m_FilePointer = fopen(filename.c_str(), "rb");
...
/*			if (//utility.FileExists(FileToPNG(filename)))
	...
	bool rval = LoadFromHandle(m_FilePointer);
	
	CleanUp(m_FilePointer);
	
	return rval;
}

bool JOEMODEL::LoadFromHandle(FILE * m_FilePointer)
{
	char strMessage[255] = {0};
